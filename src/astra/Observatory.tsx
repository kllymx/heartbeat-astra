...
import { Activity, ArrowDownToLine, ArrowRight, ArrowUpRight, AudioLines, Check, ChevronDown, ChevronLeft, ChevronRight, CircleHelp, Command, FlaskConical, Focus, GitBranch, Heart, LayoutDashboard, LoaderCircle, Maximize2, Network, Pause, Play, Radio, RotateCcw, Send, ShieldCheck, TerminalSquare, X } from 'lucide-react'
...
export default function Observatory() {
  ...
  const intelligence = <aside className="astra-intelligence">
    <div className="astra-card-heading"><span><Activity size={14}/> Signal analysis</span><span className="astra-small-tag">{connectedMode && modelStatus?.available ? 'MODEL' : 'ON DEVICE'}</span></div>
  ...
  return <div className={`astra-app ${cinema ? 'astra-app--cinema' : ''}`}>
    <header className="astra-toolbar">
      <a className="astra-brand" href="?view=observatory" aria-label="Heartbeat home"><Activity size={18}/><span>heartbeat</span></a>
      <nav className="astra-nav" aria-label="Main navigation">{NAV.map(({ id, label, icon: Icon }) => <button key={id} aria-label={label} className={view === id ? 'is-active' : ''} onClick={() => { setView(id); setPlaying(false) }} aria-current={view === id ? 'page' : undefined}><Icon size={13} strokeWidth={1.5}/><span>{label}</span></button>)}</nav>
      <div className="astra-toolbar__status"><span className="astra-source-badge"><span/>{sample ? 'SAMPLE DAY' : 'LOCAL SIGNALS'}</span>{isTauriRuntime() && <a className="astra-text-button" href="?view=classic">Telemetry console <ArrowUpRight size={12}/></a>}<button className="astra-icon-button" onClick={event => { aboutOpener.current = event.currentTarget; setShowAbout(true) }} aria-label="About this observatory"><CircleHelp size={14}/></button><button className="astra-icon-button" onClick={() => setCinema(value => !value)} aria-label={cinema ? 'Exit presentation mode' : 'Enter presentation mode'} title="Presentation mode"><Maximize2 size={14}/></button></div>
    </header>
    <main className="astra-main">
      <div className="astra-content">
        <div className="astra-page-heading"><div><h1>{view === 'overview' ? 'Heartbeat' : view === 'replay' ? 'Day replay' : view === 'memory' ? 'Memory map' : 'Pattern lab'}</h1><p>{view === 'overview' ? 'Body signals / workspace / agent activity' : view === 'replay' ? 'Explore recorded moments and their workspace context.' : view === 'memory' ? 'Connections between applications, contexts, and agent sessions.' : 'Compare the measured patterns in synthetic sample days.'}</p></div><div className="astra-heading-actions"><span className="astra-date">{dayStamp}</span><button className="astra-button" onClick={exportDay}>{exported ? <Check size={13}/> : <ArrowDownToLine size={13}/>} {exported ? 'Exported' : 'Export day'}</button></div></div>
        ...
        {view === 'memory' ? <MemoryConstellation snapshot={snapshot} onSelectTime={jumpTo}/> : view === 'lab' ? <div className="astra-lab"><div className="astra-lab__intro"><FlaskConical size={22}/><h2>Sample day comparison</h2><p>These are three designed sample days, not predictions. All measurements below are calculated from each scenario’s timeline.</p></div><div className="astra-lab__grid">{SCENARIOS.map(s => {
          ...
          <div className="astra-metrics">
            ...
            <Metric icon={<TerminalSquare size={16}/>} label="WITH AI AGENTS" value={duration(metrics.agentMinutes)} detail="Human + agent collaboration" spark="agent"/>
          </div>
